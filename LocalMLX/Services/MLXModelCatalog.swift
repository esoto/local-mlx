...
/// an entry here and clicking "Start Server" will fetch the weights
/// on first launch (into `~/.cache/huggingface/`) and reuse them after.
///
/// Entries are deliberately limited to quantised (4-bit) builds from
/// mlx-community — these are the variants most likely to fit in a
...
enum MLXModelCatalog {
    ...
    /// within each family so the picker reads naturally.
    static let builtIn: [MLXModelEntry] = [
        ...
        MLXModelEntry(
            ...
            blurb: "Very small Google model — fast loads.",
            family: .gemma,
            approxSizeGB: 1.7),
        MLXModelEntry(
